...
/**
 *
 * Funció que s'encarrega d'escriure les dades d'un node de la llista a disc.
 * Escriu la key de desti en format IATA amb 3 chars, numero de vols, i retard total.
 *
 */

void write_list_items(list_data *data, FILE *fp) {
    if (data != NULL) {
        fwrite(data->key, sizeof(char), 3, fp);
        fwrite(&data->num_times, sizeof(int), 1, fp);
...
/**
 *
 * Funció que s'encarrega d'escriure el nombre de nodes de la llista a disc
 * i cridar a la funcio write_list_items per cada item.
 *
 */
...
void write_list(list *l, FILE *fp) {
    ...
    list_item *current;
    current = l->first;
    fwrite(&l->num_items, sizeof(int), 1, fp);
    while (current != NULL){    
        write_list_items(current->data, fp);
